...
/*!
{
  ...
  "tags": ["hbbtv", "video"],
  "async" : true,
  "warnings": ["This test is currently only compatible to HbbTV >= 1.x"],
  "notes": [{
      "name": "OIPF Spec",
      "href": "http://www.oipf.tv/web-spec/volume5.html"
    },
    {
      "name": "Inspired by the MIT-xperts HBBTV testsuite",
...
define(['Modernizr', 'addTest', 'getBody', 'createElement', 'test/hbbtv'], function(Modernizr, addTest, getBody, createElement) {
  ...
  */
  var testData = window.MODERNIZR_HBBTVVIDEO_TEST_VIDEO || 'https://github.com/philippsimon/Modernizr/raw/hbbtv/test/video/black_352x288_25fps_180s_aac_low.mp4';
    ...
    setTimeout(function() {
      ...
      createTestVideo(function(video, existingVideoParams) {
        ...
        var tests = {
          ...
          seek: function() {
            try {
              video.seek(seekSeconds * 1000);
              // console.log('video.seek');
              video.play(1);
              // console.log('video.seek');

              // send additional seek for some devices who need it executed while video is playing
              var interval = setInterval(function() {
                if (PLAYSTATES[video.playState] === 'playing') {
                  clearInterval(interval);
                  if (!isSeekPosition(video, seekSeconds)) {
                    video.seek(seekSeconds * 1000);
                    // console.log('video.seek');
                  }
                }
        ...
        function isSeekPosition(video, seconds) {
          var secs = isNaN(video.playPosition) ? -1 : Math.floor(video.playPosition / 1000);
          return secs >= 0 && secs >= (seconds - 2) && secs <= (seconds + 10);
        }
        ...
        function testSeekPosition(video) {
          ...
          var interval = setInterval(function() {
            ...
            if (PLAYSTATES[video.playState] === 'playing' && isSeekPosition(video, seekSeconds)) {
              passed.seek = true;
            } else if (trials < 10) {
              return;
            }
  ...
  function createTestVideo(callback) {
    ...
    /*getElementByParams(params.parentElement).addEventListener('DOMNodeInserted', function() {
